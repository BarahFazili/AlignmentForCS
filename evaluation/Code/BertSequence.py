...
from transformers import (
    BertForSequenceClassification, BertTokenizer, XLMForSequenceClassification, XLMTokenizer, BertModel, 
...
def train(args, train_dataset, valid_dataset, model, tokenizer, labels):
    ...
    for _ in train_iterator:
        ...
        # file_ = open("./TSTG/random/logs_42_test.txt.orig", "w+")  # append mode
...
def main():
    ...
    save_path = '?.pt'
    model_0 = BertModel.from_pretrained('bert-base-multilingual-cased')#,force_download=True)
    model_0.load_state_dict(torch.load(save_path, map_location='cpu'))
    pretrained_dict = model_0.state_dict()
    model_dict = model.state_dict()
    new_pretrained_dict = {}
    for k,i in pretrained_dict.items():
        new_key = "bert." + str(k)
