...
class Model_lin(nn.Module):
  ...
  def forward(self, em_en, em_hi): #data.size()=BS*2*1*max_len
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:
                        ...
                        if results['f1']>best_f1:
                            best_f1=results['f1']
                            logger.info("New best f1 at %s",str(best_f1))
                            # if args.save_model:
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    logger.info('eval f1 at '+str(results['f1']))
    print(results)
    return results
...
def main():
    ...
    load_path = '?.pt' #Path of the saved linear layer
    
    model_head.load_state_dict(torch.load(load_path,map_location='cuda:0'))
    ll = copy.deepcopy(model_head.cont_head.state_dict())
    model.bert.cont_head.load_state_dict(ll)
