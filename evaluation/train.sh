...
if [ $TASK == 'MLM' ]; then 
  bash $CODE_DIR/train_MLM.sh $TASK $MODEL $MODEL_TYPE 
elif [ $TASK == 'LID_EN_ES' ]; then 
  bash $CODE_DIR/train_token.sh $TASK $MODEL $MODEL_TYPE $DATA_DIR $OUTPUT_DIR
elif [ $TASK == 'LID_EN_HI' ]; then
  # bash $CODE_DIR/train_token.sh "$TASK/Devanagari" $MODEL $MODEL_TYPE $DATA_DIR $OUTPUT_DIR  #originally
  bash $CODE_DIR/train_token.sh "$TASK/Romanized" $MODEL $MODEL_TYPE $DATA_DIR $OUTPUT_DIR
  ...
  # bash $CODE_DIR/train_sentence.sh "$TASK/Romanized/SemEval_3way" $MODEL $MODEL_TYPE $DATA_DIR $OUTPUT_DIR
  ...
  bash $CODE_DIR/train_qa_xlmr.sh $TASK $MODEL $MODEL_TYPE $DATA_DIR $OUTPUT_DIR
...
elif [ $TASK == 'Sentiment_EN_ML' ]; then
  bash $CODE_DIR/train_sentence.sh "$TASK" $MODEL $MODEL_TYPE $DATA_DIR $OUTPUT_DIR
elif [ $TASK == 'Sentiment_EN_TM' ]; then
  bash $CODE_DIR/train_sentence.sh "$TASK" $MODEL $MODEL_TYPE $DATA_DIR $OUTPUT_DIR
elif [ $TASK == 'ALL' ]; then
